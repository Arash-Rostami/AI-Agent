...
## 2. Three unrelated things called "session"
...
3. **`KeySessionManager`** (`utils/sessionManager.js`) — per-identity sticky provider slot for the Gemini fallback cascade (§5), persisted to `data/sessions.json`. Unrelated to chat history despite the file `middleware/keySession.js` sharing the name.

None share storage or code. See utilsPattern §1 before assuming a "session" bug touches more than one of these.
...
Controllers don't call the low-level `callGeminiAPI` directly — they call `askGemini`, which owns a **two-way fallback cascade**: `GEMINI_API_KEY` → an ArvanCloud-hosted Gemini model (now tool-calling-capable too), capped at 10s, with the winning provider persisted per-identity via `KeySessionManager` so a user's next request starts where the last one succeeded rather than re-trying a dead key. (A second native key, `GEMINI_API_KEY_ALT`, was tried and removed — its project returned a persistent `403 PERMISSION_DENIED`.) On top of that, a **global** circuit breaker (`sessionManager.isPrimaryDown()`/`markPrimaryDown()`) trips when Gemini's free tier hits its *daily* quota — every identity, not just ones that already failed over, skips the doomed `primary` attempt entirely until it cools down. Thinking mode is a separate, non-cascading, deliberately tool-less path: it calls ArvanCloud's `Gemini-3-Flash-Preview-kc6io` via the plain (no-tools) call, so the frontend hides Web Search and attachments whenever it's toggled on. See servicesPattern §3a for the full mechanics.

Tools themselves split schema (`tools/`, pure data) from implementation (`services/*Tool.js`, plain functions) with a manual, unenforced sync point (`tools/toolDefinitions.js`) and dual mode-gating (offer-time in `formatter.js`, execution-time in `responseHandler.js`) that must be updated together.
...
- **Which "session" you're touching** (§2) — chat history, login, or API-key rotation are three separate systems.
