...
router.post('/upload-avatar', protect, avatarUpload.single('avatar'), async (req, res) => {
    try {
        ...
        if (user) {
            // Store relative path
            user.avatar = `/assets/uploads/avatars/${req.file.filename}`;
            await user.save();
...
router.get('/me', async (req, res) => {
    try {
        ...
        if (!token) return res.json({username: null, canSync: false, avatar: null});

        const decoded = jwt.verify(token, JWT_SECRET);
        const user = await User.findById(decoded.id).select('username avatar');

        if (!user) return res.json({username: null, canSync: false, avatar: null});
        ...
        res.json({username: null, canSync: false, avatar: null});
    }
});

// Deprecated: keeping for backward compatibility if needed, but aliasing /me logic
