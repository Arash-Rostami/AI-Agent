...
export const restoreInteraction = async (req, res) => {
    try {
        ...
        // ✏️ CHANGED: Removed syncToDatabase call to ensure "Lazy Save" (restored chat is not saved until user interacts)
...
export const restoreChat = (log) => {
    return log.messages.map(msg => {
        ...
        // ✏️ CHANGED: Added fallback to use `content` field if `parts` is missing (supports legacy data)
